...
M.get_nodes = function(query, lang, defaults, bufnr)
    ...
    for match in ts_query.iter_prepared_matches(parsed_query, root, bufnr,
        ...
        locals.recurse_local_nodes(match, function(_, node, path)
            ...
            if name == nil then name = defaults[type] or 'empty' end
...
--   type: string
-- }
M.get_all_nodes = function(query, lang, _, bufnr, pos_row, _)
    bufnr = bufnr or 0
    -- todo a huge number
...
M.nodes_at_cursor = function(query, default, bufnr, row, col)
    ...
    if nodes == nil or #nodes == 0 then
        print('Unable to find any nodes at pos. ' .. tostring(row) .. ':' ..
                  tostring(col))
        return nil
    end
